...
**No mid-round MSE fitting in this run.** Mid-round numerical fitting of candidate-law parameters against your collected trajectories is **not available** in this configuration. Reason about parameter values qualitatively from your observations during the rounds. All numerical optimisation of your law's free parameters will be performed automatically by the evaluator at submission time via the `fit_parameters()` mechanism described in the Final Submission section below — declare your uncertain constants there and the evaluator will tune them. Any `<run_mse_fit>` tag emitted during the rounds will be rejected.
