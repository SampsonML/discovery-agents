...
for opus-4-7 and gpt-5.5, comparing guided (with-mse) vs no_mse (without-mse).

Takes eight results directories in this exact order:

    2-rounds-guided  2-rounds-no_mse
    4-rounds-guided  4-rounds-no_mse
    8-rounds-guided  8-rounds-no_mse
    16-rounds-guided 16-rounds-no_mse

Each (model, condition) is pooled across all worlds and seeds inside its
directory. Color = model, marker = with-mse (circle) vs without-mse (X).
Output goes to `analysis_rounds/round_analysis_no_mse.{png,pdf}` in the repo
root.

Usage:
    python scripts/round_analysis_no_mse.py r2g r2nm r4g r4nm r8g r8nm r16g r16nm
"""
...
_GUIDED = "with-mse"
_NO_MSE = "without-mse"
_CONDITIONS = (_GUIDED, _NO_MSE)
_MARKERS = {_GUIDED: "o", _NO_MSE: "X"}
_LINESTYLES = {_GUIDED: "-", _NO_MSE: "--"}
...
def make_plot(
    ...
    if expected_pass_k is not None:
        ...
        out_stem = f"round_analysis_no_mse_expected_passed_at_{expected_pass_k}"
        ...
        out_stem = f"round_analysis_no_mse_passed_at_{pass_k}"
    else:
        ...
        out_stem = "round_analysis_no_mse"
...
def main() -> int:
    ...
    parser.add_argument(
        "dirs", nargs=8,
        metavar="DIR",
        help="Eight result directories in order: r2g r2nm r4g r4nm r8g r8nm r16g r16nm",
    )
    parser.add_argument(
        "--out",
